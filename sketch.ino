...
bool zEvidence=false,sEvidence=false,modelValid=false,oledOK=false;
...
void process(float elapsed){
 ...
 zEvidence=dz < -8 && dh < -3;sEvidence=ds>4;
 ...
 abstain=uncertain;
 bool candidate=zEvidence&&sEvidence&&pSite>=.75f&&!uncertain;
 if(candidate)persist+=elapsed;else persist=0;
 ...
 else if(changed||uncertain){state=WATCH;reason=candidate?"ACCUMULATING EVIDENCE":uncertain?"UNCERTAIN: ABSTAIN":"INSUFFICIENT CORROBORATION";}
 ...
 if(state!=previous||millis()-lastReceipt>=2000){receipt();lastReceipt=millis();previous=state;}
}
void render(){
...
void setup(){
 ...
 Serial.println("A acknowledges/mutes latched alert; R requests guarded reacquisition; ? prints evidence.");
}
void loop(){
 while(Serial.available()) {char c=toupper(Serial.read());if(c&&strchr("NMLSIF",c))phantom.write(c);
 if(c=='A'&&latched)acknowledged=true;
 if(c=='R'){baseline=false;resetAcquisition();persist=0;}
 if(c=='?')receipt();}
 if(!digitalRead(ACK)&&latched)acknowledged=true;
 while(phantom.available()){
 char c=phantom.read();if(c=='\n'){
 Sample next;int consumed=0;
 if(sscanf(line.c_str(),"%u,%f,%f,%f,%f,%f,%f%n",&next.seq,&next.lf,&next.hf,&next.strain,&next.temp,&next.motion,&next.contact,&consumed)==7&&consumed==line.length()
